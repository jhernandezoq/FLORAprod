  ...
  <div class="modal fade" role="dialog" id="motivorechazo">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{url('anticipos/rechazarlegalizaciongastosuser/')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                    ...
                    <table class="table-responsive-md table-bordered table-striped table-sm" cellspacing="0" width="100%">
                        ...
                        @foreach($anticipos as $anticipo)
                              ...
                              <td>
                                ...
                                <a href="{{url('anticipos/')}}/{{$anticipo->id}}/{{$user}}/aceptargastosuser" 
                                  class="btn btn-success">Aprobar</a>
                              </td>
                              <td>
                                <input type='button' class='btn btn-danger' value='Rechazar' 
                                  style='color:white;' onclick='CargarMotivoRechazo("{{$anticipo->id}}","{{$user}}")'>
                              </td>
                            </tr>
                        @endforeach
                        </tbody>
...
function CargarMotivoRechazo(id,user_id){
  var token=$("input[name=_token]").val();
